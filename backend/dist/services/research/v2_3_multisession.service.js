...
exports.v2_3_multiSessionService = exports.V2_3_MultiSessionService = void 0;
const v2_2_fresh_validation_service_1 = require("./v2_2_fresh_validation.service");
const V2_3_DISCLAIMER = 'Strategy V2.3 Multi-Session Lab is a rigorous multi-session empirical validation system in DEMO/PAPER mode only. No strategy guarantees future returns. Production Strategy V2 remains unmodified.';
class V2_3_MultiSessionService {
    inMemoryMultiSessionDataset = [];
     ...
     */
    async getOrSeedMultiSessionDataset(sessionsCount = 10, tradesPerSession = 55) {
    ...
    computeSessionMetrics(trades) {
        ...
        Object.entries(sessionMap).forEach(([sId, sTrades]) => {
            ...
            let sessionOutcome = 'NEUTRAL';
            if (totalPnL > 1.0 && winRate >= 58.0)
                sessionOutcome = 'POSITIVE';
            else if (totalPnL < 0 || winRate < 50.0)
                sessionOutcome = 'NEGATIVE';
     ...
     */
    evaluateHypothesisConsistency(hypothesisId, allTrades, sessionsList) {
        let hypothesisName = '';
        let controlVariant = '';
        ...
        const relevantTrades = allTrades.filter(t => t.experimentVersion.includes(hypothesisId === 'HYPOTHESIS_A' ? '30S' : (hypothesisId === 'HYPOTHESIS_B' ? 'CONFLUENCE' : '80')) ||
            (t.experimentVersion === 'V2_BASELINE' || t.experimentVersion === 'V2.3_LOW_REGIME_70'));
        const expTrades = allTrades.filter(t => t.experimentVersion.includes(hypothesisId === 'HYPOTHESIS_A' ? '30S' : (hypothesisId === 'HYPOTHESIS_B' ? 'CONFLUENCE' : '80')));
        const ctrlTrades = allTrades.filter(t => t.experimentVersion === (hypothesisId === 'HYPOTHESIS_C' ? 'V2.3_LOW_REGIME_70' : 'V2_BASELINE'));
        ...
        dedicatedSessions.forEach(s => {
            ...
            if (s.sessionOutcome === 'POSITIVE')
                positiveSessions++;
            else if (s.sessionOutcome === 'NEGATIVE')
                negativeSessions++;
            else
                neutralSessions++;
        });
        ...
        const winRateCI = v2_2_fresh_validation_service_1.v2_2_freshValidationService.calculateConfidenceInterval95(expWins, expTrades.length);
        const expectancyCI = {
        ...
        let promotionGateStatus = 'CONTINUE_RESEARCH';
     ...
     */
    computeCrossAssetMetrics(trades) {
        const assets = ['R_10', 'R_25', 'R_50', 'R_75', 'R_100'];
        const result = {};
        assets.forEach(asset => {
            ...
            const expectancy = total > 0 ? Math.round(((wins / total * avgWin) - (losses / total * avgLoss)) * 10000) / 10000 : 0;
            const ci = v2_2_fresh_validation_service_1.v2_2_freshValidationService.calculateConfidenceInterval95(wins, total);
            result[asset] = {
                asset,
                tradesCount: total,
                wins,
     ...
     */
    computeCrossRegimeMetrics(trades) {
        const regimes = ['TRENDING_UP', 'TRENDING_DOWN', 'RANGING', 'HIGH_VOLATILITY', 'COMPRESSION', 'LOW_VOLATILITY'];
        const result = {};
     ...
     */
    evaluateMultiSessionOOS(trades) {
     ...
     */
    async getMultiSessionDashboard() {
...
exports.V2_3_MultiSessionService = V2_3_MultiSessionService;
exports.v2_3_multiSessionService = new V2_3_MultiSessionService();
