...
const PORTFOLIO_DISCLAIMER = 'Portfolio simulations evaluate multi-asset allocation, cross-strategy diversification, and aggregate drawdowns using simulated historical data. Past correlations may break down under market stress and do not guarantee future performance.';
class PortfolioService {
    inMemoryPortfolios = new Map();
    /**
     ...
     */
    async calculateCorrelationMatrix(assets, timeframe = '5m', candleCount = 100) {
    ...
    async createPortfolio(config) {
        ...
        const id = `port_${(0, crypto_1.randomUUID)().replace(/-/g, '').substring(0, 16)}`;
        const maxDrawdown = config.maxPortfolioDrawdownPercent || 15.0;
        const fullAllocations = config.allocations.map((a) => ({
            asset: a.asset,
            strategy: a.strategy,
        ...
        try {
            await database_1.pool.query(`INSERT INTO portfolios (id, user_id, name, initial_capital, current_equity, max_portfolio_drawdown, status)
         VALUES (?, ?, ?, ?, ?, ?, 'ACTIVE')`, [id, config.userId, config.name, config.initialCapital, config.initialCapital, maxDrawdown]);
            for (const alloc of fullAllocations) {
                await database_1.pool.query(`INSERT INTO portfolio_positions (id, portfolio_id, asset, strategy, weight_pct, allocated_capital, current_pnl)
           VALUES (?, ?, ?, ?, ?, ?, 0.00)`, [
                    `pos_${(0, crypto_1.randomUUID)().replace(/-/g, '').substring(0, 16)}`,
                    id,
                    alloc.asset,
                    alloc.strategy,
                    alloc.weightPercent,
     ...
     */
    async simulatePortfolio(portfolioId, timeframe = '5m', candleCount = 100) {
        let portfolio = this.inMemoryPortfolios.get(portfolioId);
        if (!portfolio) {
            // Try to load from database
            const [rows] = await database_1.pool.query(`SELECT * FROM portfolios WHERE id = ?`, [portfolioId]);
            if (rows.length === 0) {
                throw new Error(`Portfolio ${portfolioId} not found.`);
            }
            const pRow = rows[0];
            const [posRows] = await database_1.pool.query(`SELECT * FROM portfolio_positions WHERE portfolio_id = ?`, [portfolioId]);
            const allocations = posRows.map((r) => ({
                asset: r.asset,
                strategy: r.strategy,
        ...
        const unallocatedCash = portfolio.initialCapital - portfolio.allocations.reduce((sum, a) => sum + a.allocatedCapital, 0);
        ...
        let riskLimitMessage = undefined;
            ...
            for (const ts of sortedTimestamps) {
                ...
                for (const { alloc, bt } of positionResults) {
                    const closestEq = (bt.equityCurve || [])
                        .filter((e) => new Date(e.timestamp).getTime() <= new Date(ts).getTime())
                        .pop();
        ...
        const returnPercent = portfolio.initialCapital > 0
        ...
        if (portfolioEquityCurve.length > 2) {
            ...
            for (let i = 1; i < portfolioEquityCurve.length; i++) {
                const r = (portfolioEquityCurve[i].equity - portfolioEquityCurve[i - 1].equity) /
    ...
    async listPortfolios(userId = 1) {
        ...
        try {
            const [rows] = await database_1.pool.query(`SELECT * FROM portfolios WHERE user_id = ?`, [userId]);
            for (const row of rows) {
                if (!list.some((item) => item.id === row.id)) {
                    const [posRows] = await database_1.pool.query(`SELECT * FROM portfolio_positions WHERE portfolio_id = ?`, [row.id]);
                    list.push({
                        id: row.id,
                        userId: row.user_id,
                        name: row.name,
