...
class ExperimentService {
    inMemoryExperiments = new Map();
    inMemoryTrades = new Map();
    /**
    ...
    async createExperiment(config) {
        const id = `exp_${(0, crypto_1.randomUUID)().replace(/-/g, '').substring(0, 16)}`;
        ...
        try {
            await database_1.pool.query(`INSERT INTO paper_experiments (id, user_id, name, strategy, asset, timeframe, config_hash, parameters, status, start_balance, current_balance)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'CREATED', ?, ?)`, [
                id,
                config.userId,
                config.name,
    ...
    computeGranularHashes(config) {
        ...
        return {
            configHash,
            strategyHash: (0, crypto_1.createHash)('sha256').update(config.strategy).digest('hex').substring(0, 16),
            datasetHash: (0, crypto_1.createHash)('sha256').update(`${config.asset}_${config.timeframe}`).digest('hex').substring(0, 16),
            indicatorConfigHash: (0, crypto_1.createHash)('sha256').update(JSON.stringify(config.parameters || {})).digest('hex').substring(0, 16),
            riskConfigHash: (0, crypto_1.createHash)('sha256').update(String(config.startBalance || 10000)).digest('hex').substring(0, 16)
        };
    }
    /**
     * Captures a periodic experiment snapshot for longitudinal paper tracking.
     */
    async takeSnapshot(experimentId) {
        ...
        try {
            await database_1.pool.query(`INSERT INTO experiment_snapshots (experiment_id, equity, pnl, drawdown, exposure, trades_count, signals_count, risk_state, market_state)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
     ...
     */
    async updateStatus(experimentId, newStatus) {
        const exp = await this.getExperiment(experimentId);
        if (!exp) {
            throw new Error(`Experiment ${experimentId} not found.`);
        }
        ...
        try {
            ...
            await database_1.pool.query(`UPDATE paper_experiments SET status = ?, completed_at = ? WHERE id = ?`, [newStatus, completedDate, experimentId]);
        }
        catch (err) {
    ...
    async logTrade(tradeData) {
        ...
        // Calculate trade PnL
        const returnRate = tradeData.direction === 'BUY'
        ...
        const trade = {
            id: `pt_${(0, crypto_1.randomUUID)().replace(/-/g, '').substring(0, 16)}`,
        ...
        try {
            await database_1.pool.query(`INSERT INTO paper_experiment_trades (id, experiment_id, direction, entry_price, exit_price, amount, pnl, slippage, fees, result, journal_notes, entry_time, exit_time)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                trade.id,
                trade.experimentId,
                trade.direction,
            ...
            await database_1.pool.query(`UPDATE paper_experiments SET current_balance = ?, total_trades = ?, win_rate = ?, pnl = ? WHERE id = ?`, [exp.currentBalance, exp.totalTrades, exp.winRate, exp.pnl, exp.id]);
        }
        catch (err) {
    ...
    async getExperiment(id) {
        ...
        try {
            const [rows] = await database_1.pool.query(`SELECT * FROM paper_experiments WHERE id = ?`, [id]);
            if (rows.length === 0)
                return null;
            const r = rows[0];
            const exp = {
     ...
     */
    async compareBacktestVsPaper(experimentId) {
        const exp = await this.getExperiment(experimentId);
        if (!exp) {
            throw new Error(`Experiment ${experimentId} not found.`);
        }
        ...
        const paperProfitFactor = grossLosses > 0 ? Math.round((grossWins / grossLosses) * 100) / 100 : grossWins > 0 ? 99.0 : 0;
        ...
        const slippageDragPercent = grossVolume > 0 ? Math.round((totalSlippageCost / grossVolume) * 100 * 100) / 100 : 0;
        const winRateDeviation = Math.round((paperWinRate - bt.winRate) * 100) / 100;
        let divergenceAssessment = 'CONSISTENT';
     ...
     */
    async listExperiments(userId = 1) {
        const list = [];
        ...
        try {
            const [rows] = await database_1.pool.query(`SELECT * FROM paper_experiments WHERE user_id = ? ORDER BY created_at DESC`, [userId]);
            for (const r of rows) {
