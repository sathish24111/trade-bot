...
exports.strategyEnsembleService = exports.StrategyEnsembleService = void 0;
...
class StrategyEnsembleService {
    ...
    async getEnsembleConfig(id = 'ENSEMBLE_CORE_3') {
        const mem = this.ensembleConfigs.get(id);
        if (mem)
            return mem;
        try {
            const [rows] = await database_1.pool.query('SELECT * FROM research_ensemble_configs WHERE id = ?', [id]);
            if (rows.length > 0) {
                const r = rows[0];
                const config = {
                    id: r.id,
    ...
    async saveEnsembleConfig(config) {
        if (!config.id) {
            config.id = `ENS_${Date.now()}_${crypto_1.default.randomBytes(2).toString('hex')}`;
        }
        ...
        try {
            await database_1.pool.query(`INSERT INTO research_ensemble_configs (id, name, aggregation_mode, strategies, min_confirmations, enabled)
         VALUES (?, ?, ?, ?, ?, ?)
     ...
     */
    async evaluateEnsemble(asset, ensembleId = 'ENSEMBLE_CORE_3') {
        const config = await this.getEnsembleConfig(ensembleId);
        const votes = [];
        // 1. Collect component signals
        for (const item of config.strategies) {
            const evalRes = strategy_service_1.strategyEngine.evaluate(item.strategyId, asset);
            votes.push({
        ...
        switch (config.aggregationMode) {
            ...
            case 'WEIGHTED': {
                ...
                for (const v of votes) {
                    if (v.signal === 'BUY')
                        buyWeight += v.weight * (v.confidence / 100);
                    if (v.signal === 'SELL')
                        sellWeight += v.weight * (v.confidence / 100);
                }
        ...
        if (hasConflict) {
            await this.recordConflict({
                ...
                regime: asset.marketRegime || asset.trend || 'UNKNOWN',
    ...
    async recordConflict(conflict) {
        ...
        if (this.inMemoryConflicts.length > 100)
            this.inMemoryConflicts.pop();
        try {
            const [res] = await database_1.pool.query(`INSERT INTO research_signal_conflicts (ensemble_id, asset, regime, signals, resolved_signal, resolution_method, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [
    ...
    async getConflicts(asset, limit = 20) {
        try {
...
exports.StrategyEnsembleService = StrategyEnsembleService;
exports.strategyEnsembleService = new StrategyEnsembleService();
