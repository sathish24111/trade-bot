...
exports.paperResearchComparisonService = exports.PaperResearchComparisonService = void 0;
const database_1 = require("../../config/database");
class PaperResearchComparisonService {
    /**
     * Compares strategy metrics across Backtest, Validation, OOS, Walk-Forward, and Paper stages
     */
    async getPerformanceStagesComparison(experimentId = 'EXP_DEFAULT', strategyId = 'EMA_RSI') {
        // 1. Check if database has recorded performance stages
        try {
            const [rows] = await database_1.pool.query(`SELECT stage, win_rate, return_pct, max_drawdown, expectancy, profit_factor, trade_count, sample_quality, slippage, fees, risk_utilization
         FROM performance_stage_metrics
         WHERE experiment_id = ?
         ORDER BY FIELD(stage, 'BACKTEST', 'VALIDATION', 'OOS', 'WALK_FORWARD', 'PAPER')`, [experimentId]);
            if (rows.length >= 3) {
                const stages = rows.map(r => ({
                    ...
                    sampleQuality: r.sample_quality,
...
exports.PaperResearchComparisonService = PaperResearchComparisonService;
exports.paperResearchComparisonService = new PaperResearchComparisonService();
