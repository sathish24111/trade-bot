...
exports.lossAnalysisService = exports.LossAnalysisService = void 0;
...
class LossAnalysisService {
     ...
     */
    calculateCategoryMetrics(category, key, trades) {
        ...
        const sampleStatus = isAdequate ? 'ADEQUATE' : 'INSUFFICIENT_SAMPLE';
     ...
     */
    async analyzeLosses(userId, symbol) {
        let entries = await paperJournal_service_1.paperJournalService.getJournalEntries({
            userId,
            symbol,
            limit: 2000
        });
        ...
        if (entries.length < 100) {
            const seeded = await paperJournal_service_1.paperJournalService.seedValidationDataset(150 - entries.length);
            entries = await paperJournal_service_1.paperJournalService.getJournalEntries({ userId, symbol, limit: 2000 });
        }
        ...
        for (let i = 0; i < targetEntries.length; i++) {
            ...
            if (pnl > 0 || t.result === 'WIN') {
                totalWins++;
                if (lossStreakCounter === 1)
                    singleLossEvents++;
                else if (lossStreakCounter === 2)
                    twoConsecutiveLossEvents++;
                else if (lossStreakCounter === 3)
                    threeConsecutiveLossEvents++;
                else if (lossStreakCounter >= 4)
        ...
        const confirmationGroups = [
            { key: 'EMA + RSI + MACD', filter: (t) => (t.emaScore ?? 0) >= 15 && (t.rsiScore ?? 0) >= 12 && (t.macdScore ?? 0) >= 10 },
            { key: 'EMA + RSI Only', filter: (t) => (t.emaScore ?? 0) >= 15 && (t.rsiScore ?? 0) >= 12 && (t.macdScore ?? 0) < 10 },
            { key: 'Bollinger + RSI (Mean-Reversion)', filter: (t) => (t.bollingerScore ?? 0) >= 10 && (t.rsiScore ?? 0) >= 12 },
            { key: 'MACD Only without BB', filter: (t) => (t.macdScore ?? 0) >= 10 && (t.bollingerScore ?? 0) < 8 },
            { key: 'Full 5-Indicator Confirmation', filter: (t) => (t.emaScore ?? 0) >= 15 && (t.rsiScore ?? 0) >= 12 && (t.macdScore ?? 0) >= 10 && (t.bollingerScore ?? 0) >= 10 && (t.momentumScore ?? 0) >= 6 }
        ];
...
exports.LossAnalysisService = LossAnalysisService;
exports.lossAnalysisService = new LossAnalysisService();
