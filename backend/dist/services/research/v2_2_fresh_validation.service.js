...
exports.v2_2_freshValidationService = exports.V2_2_FreshValidationService = void 0;
const V2_2_DISCLAIMER = 'Strategy V2.2 Fresh Validation is an independent scientific verification in DEMO/PAPER mode only. Observations do not guarantee future profitability. Production Strategy V2 remains unmodified.';
class V2_2_FreshValidationService {
    inMemoryFreshDataset = [];
    /**
     ...
     */
    async getOrSeedFreshDataset(count = 160) {
        ...
        const variants = ['V2_BASELINE', 'V2.2_HIGH_VOL_30S', 'V2.2_RANGING_CONFLUENCE', 'V2.2_LOW_REGIME_80'];
        const freshTrades = [];
        const now = Date.now();
     ...
     */
    computeVariantMetrics(id, label, role, condition, parameterDescription, trades, totalOpportunities, rejectedCount, rejectionReasons) {
        ...
        let promotionStatus = 'INSUFFICIENT_SAMPLE';
    ...
    computeBreakdowns(trades) {
        const buildCategory = (key, category, subTrades) => {
     ...
     */
    evaluateFreshOOSValidation(trades) {
        ...
        const degradation = train.winRate > 0
            ...
            : 0;
        const verdict = (degradation <= 15.0 && oos.winRate >= 55.0) ? 'OOS_VALIDATED' : 'MARGINAL';
        return {
     ...
     */
    async getFreshValidationDashboard() {
        ...
        // Matrix
        const controlMetrics = this.computeVariantMetrics('V2_BASELINE', 'V2_BASELINE', 'CONTROL', 'ALL_REGIMES', 'Existing V2 Baseline behavior', controlTrades, controlTrades.length, 0, {});
        const expAMetrics = this.computeVariantMetrics('V2.2_HIGH_VOL_30S', 'V2.2_HIGH_VOL_30S', 'EXPERIMENT', 'HIGH_VOLATILITY', '30-second duration under High Volatility', expATrades, expATrades.length, 0, {});
        ...
        const expBMetrics = this.computeVariantMetrics('V2.2_RANGING_CONFLUENCE', 'V2.2_RANGING_CONFLUENCE', 'EXPERIMENT', 'RANGING', 'MACD + Bollinger Confluence (%B < 0.15 / > 0.85)', expBAccepted, expBTrades.length, expBRejections, { RANGING_BOLLINGER_FILTER: expBRejections });
        ...
        const expCMetrics = this.computeVariantMetrics('V2.2_LOW_REGIME_80', 'V2.2_LOW_REGIME_80', 'EXPERIMENT', 'RANGING/COMPRESSION', 'Score threshold >= 80 in low regimes', expCAccepted, expCTrades.length, expCRejections, { SCORE_BELOW_80_THRESHOLD: expCRejections });
...
exports.V2_2_FreshValidationService = V2_2_FreshValidationService;
exports.v2_2_freshValidationService = new V2_2_FreshValidationService();
