...
exports.tradeDiagnosticsService = exports.TradeDiagnosticsService = void 0;
const database_1 = require("../../config/database");
class TradeDiagnosticsService {
    inMemoryDiagnostics = new Map();
    /**
     * Generates or retrieves an explainable diagnostic record for a simulated trade
     */
    async getTradeDiagnostic(tradeId) {
        const mem = this.inMemoryDiagnostics.get(tradeId);
        if (mem)
            return mem;
        // 1. Try fetching from MySQL
        try {
            const [rows] = await database_1.pool.query('SELECT * FROM trade_diagnostics WHERE trade_id = ?', [tradeId]);
            if (rows.length > 0) {
                const r = rows[0];
                const record = {
                    tradeId: r.trade_id,
    ...
    async createDiagnosticForTrade(params) {
        ...
        try {
            await database_1.pool.query(`INSERT INTO trade_diagnostics (trade_id, asset, strategy_id, trade_signal, entry_price, exit_price, amount, pnl, result, market_regime, indicators, decision_path, mae, mfe, slippage, fees, holding_time_seconds)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
exports.TradeDiagnosticsService = TradeDiagnosticsService;
exports.tradeDiagnosticsService = new TradeDiagnosticsService();
