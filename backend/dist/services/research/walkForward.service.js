...
exports.walkForwardService = exports.WalkForwardService = void 0;
...
const WF_DISCLAIMER = 'Walk-forward analysis tests optimized parameters strictly on unseen rolling future periods. This demonstrates historical stability under regime shifts and does NOT guarantee profit.';
class WalkForwardService {
    /**
     * Runs rolling or anchored window walk-forward validation with zero look-ahead bias.
     */
    async runWalkForward(config) {
        const { userId, asset, timeframe = '5m', strategy, parameterRanges, trainCandles = 50, testCandles = 20, stepCandles = 20, initialBalance = 10000, tradeAmount = 100, walkForwardMethod = 'ROLLING', minWindowsRequired = 3 } = config;
        ...
        if (candles.length < minRequired) {
            throw new Error(`Insufficient candles for walk-forward testing. Available: ${candles.length}, Required at least: ${minRequired}`);
        }
        // Generate combinations (cap at 30 per window to keep CPU fast)
        let combinations = optimization_service_1.optimizationService.generateCombinations(parameterRanges);
        ...
        while (currentStartIndex + trainCandles + testCandles <= candles.length) {
            ...
            let bestInSampleMetrics = metrics_service_1.metricsService.calculateMetrics([], [], initialBalance);
            for (const params of combinations) {
                const isBacktest = await backtesting_service_1.backtestingService.runBacktest({
                    userId,
                    asset,
                    timeframe,
                    strategy,
                ...
                const isMetrics = metrics_service_1.metricsService.calculateMetrics(isBacktest.trades, isBacktest.equityCurve, initialBalance);
            ...
            const oosBacktest = await backtesting_service_1.backtestingService.runBacktest({
                userId,
                asset,
                timeframe,
                strategy,
            ...
            const oosMetrics = metrics_service_1.metricsService.calculateMetrics(oosBacktest.trades, oosBacktest.equityCurve, initialBalance);
        ...
        for (const trade of allOosTrades) {
            ...
            if (cumBalance > peakEquity)
                peakEquity = cumBalance;
        ...
        const windowGuardWarning = windows.length < minWindowsRequired
     ...
     */
    async persistWalkForwardRun(run) {
        const connection = await database_1.pool.getConnection();
        try {
            await connection.beginTransaction();
            await connection.query(`INSERT INTO walk_forward_runs (
          ...
          walk_forward_efficiency, cumulative_oos_pnl, cumulative_oos_return_pct
            ...
            for (const w of run.windows) {
                await connection.query(`INSERT INTO walk_forward_results (
...
exports.WalkForwardService = WalkForwardService;
exports.walkForwardService = new WalkForwardService();
