...
exports.researchOrchestratorService = exports.ResearchOrchestratorService = void 0;
...
const paperResearchComparison_service_1 = require("./paperResearchComparison.service");
const parameterStability_service_1 = require("./parameterStability.service");
const regime_service_1 = require("./regime.service");
const riskAttribution_service_1 = require("./riskAttribution.service");
const strategyCorrelation_service_1 = require("./strategyCorrelation.service");
...
class ResearchOrchestratorService {
    ...
    async loadActiveJobsFromDb() {
        try {
            const [rows] = await database_1.pool.query(`SELECT * FROM research_jobs WHERE status IN ('QUEUED', 'RUNNING', 'PAUSED', 'RETRYING') ORDER BY priority DESC, created_at ASC`);
            for (const r of rows) {
                const job = {
                    jobId: r.job_id,
    ...
    async createJob(params) {
        ...
        const jobId = `JOB_${Date.now()}_${crypto_1.default.randomBytes(3).toString('hex')}`;
        ...
        // Persist to MySQL
        try {
            await database_1.pool.query(`INSERT INTO research_jobs (job_id, experiment_id, type, priority, status, progress, config_hash, parameters, parent_job_id, dependency_job_id)
         VALUES (?, ?, ?, ?, 'QUEUED', 0, ?, ?, ?, ?)`, [jobId, experimentId, job.type, priority, configHash, JSON.stringify(parameters), params.parentJobId || null, params.dependencyJobId || null]);
        }
        catch { }
    ...
    async getJob(jobId) {
        ...
        if (memory)
            return memory;
        try {
            const [rows] = await database_1.pool.query('SELECT * FROM research_jobs WHERE job_id = ?', [jobId]);
            if (rows.length > 0) {
                const r = rows[0];
                const job = {
                    jobId: r.job_id,
    ...
    async cancelJob(jobId, reason = 'User requested cancellation') {
        ...
        try {
            await database_1.pool.query(`UPDATE research_jobs SET status = 'CANCELLED', cancellation_reason = ?, completed_at = NOW() WHERE job_id = ?`, [reason, jobId]);
        }
        catch { }
        (0, websocket_server_1.broadcastEvent)({
    ...
    async pauseJob(jobId) {
        ...
        job.status = 'PAUSED';
        try {
            await database_1.pool.query(`UPDATE research_jobs SET status = 'PAUSED' WHERE job_id = ?`, [jobId]);
        }
        catch { }
        (0, websocket_server_1.broadcastEvent)({
    ...
    async resumeJob(jobId) {
        ...
        job.status = 'QUEUED';
        try {
            await database_1.pool.query(`UPDATE research_jobs SET status = 'QUEUED' WHERE job_id = ?`, [jobId]);
        }
        catch { }
        (0, websocket_server_1.broadcastEvent)({
    ...
    async processQueue() {
        ...
        try {
            while (this.runningCount < this.maxConcurrentJobs) {
                ...
                if (!nextJob)
                    break; // No executable jobs right now
                this.runningCount++;
    ...
    async executeJob(job) {
        ...
        try {
            await database_1.pool.query(`UPDATE research_jobs SET status = 'RUNNING', started_at = NOW() WHERE job_id = ?`, [job.jobId]);
        }
        catch { }
        (0, websocket_server_1.broadcastEvent)({
        ...
        try {
            ...
            switch (job.type) {
                case 'FULL_RESEARCH_PIPELINE':
                    ...
                    break;
                case 'BACKTEST':
                    result = await backtesting_service_1.backtestingService.runBacktest({
                    ...
                    break;
                case 'WALK_FORWARD':
                    result = await walkForward_service_1.walkForwardService.runWalkForward({
                        userId: 1,
                        strategy: job.parameters.strategy || 'EMA_RSI',
                        asset: job.parameters.asset || 'BTC/USD',
                        timeframe: job.parameters.timeframe || '5m',
                ...
                case 'PARAMETER_STABILITY':
                    result = await parameterStability_service_1.parameterStabilityService.analyzeParameterStability({
                    ...
                    break;
                case 'MONTE_CARLO':
                    result = await monteCarlo_service_1.monteCarloService.runSimulation({
                ...
                case 'REGIME_ANALYSIS':
                    result = regime_service_1.regimeService.classifyRegime(job.parameters.candles || [], Math.max(0, (job.parameters.candles?.length || 1) - 1));
                    break;
                case 'CORRELATION_ANALYSIS':
                    result = await strategyCorrelation_service_1.strategyCorrelationService.calculateStrategyCorrelationMatrix({
                        asset: job.parameters.asset || 'BTC/USD',
                        timeframe: job.parameters.timeframe || '5m'
                    });
                    break;
                case 'PORTFOLIO_SIMULATION':
                    result = riskAttribution_service_1.riskAttributionService.calculateRiskAttribution({
                ...
                case 'PAPER_COMPARISON':
                    result = await paperResearchComparison_service_1.paperResearchComparisonService.getPerformanceStagesComparison(job.experimentId);
                    break;
                default:
            ...
            job.result = result;
            try {
                await database_1.pool.query(`UPDATE research_jobs SET status = 'COMPLETED', progress = 100, result = ?, completed_at = NOW() WHERE job_id = ?`, [JSON.stringify(result), job.jobId]);
            }
            catch { }
            (0, websocket_server_1.broadcastEvent)({
            ...
            if (isTransient && (job.retryCount || 0) < 3) {
                ...
                try {
                    await database_1.pool.query(`UPDATE research_jobs SET status = 'RETRYING', retry_count = ? WHERE job_id = ?`, [job.retryCount, job.jobId]);
                }
                catch { }
            }
            else {
                ...
                job.completedAt = new Date().toISOString();
                try {
                    await database_1.pool.query(`UPDATE research_jobs SET status = 'FAILED', error = ?, completed_at = NOW() WHERE job_id = ?`, [err.message, job.jobId]);
                }
                catch { }
                (0, websocket_server_1.broadcastEvent)({
    ...
    async runFullPipeline(job) {
        ...
        for (let i = 0; i < stages.length; i++) {
            ...
            try {
                await database_1.pool.query(`INSERT INTO research_job_checkpoints (job_id, stage, progress, checkpoint_data)
           VALUES (?, ?, ?, ?)`, [job.jobId, s.stage, progress, JSON.stringify(job.checkpoint)]);
                await database_1.pool.query(`UPDATE research_jobs SET progress = ?, last_completed_stage = ?, checkpoint = ? WHERE job_id = ?`, [progress, s.stage, JSON.stringify(job.checkpoint), job.jobId]);
            }
            catch { }
            (0, websocket_server_1.broadcastEvent)({
            ...
            try {
                if (s.stage === 'BACKTEST') {
                    s.output = await backtesting_service_1.backtestingService.runBacktest({
                        strategy,
                        asset,
                        timeframe,
                        candleCount: 80,
                    ...
                    s.output = await parameterStability_service_1.parameterStabilityService.analyzeParameterStability({
                        strategyId: strategy,
                    ...
                    s.output = await monteCarlo_service_1.monteCarloService.runSimulation({
...
exports.researchOrchestratorService = new ResearchOrchestratorService();
