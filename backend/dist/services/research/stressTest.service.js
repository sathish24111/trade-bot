...
exports.stressTestService = exports.StressTestService = void 0;
const crypto_1 = require("crypto");
const database_1 = require("../../config/database");
const backtesting_service_1 = require("../backtesting.service");
const STRESS_TEST_DISCLAIMER = 'Stress testing evaluates simulated paper trading outcomes under adverse transaction costs and execution friction. It does not predict future real-market slippage or broker conditions.';
class StressTestService {
    /**
     * Evaluates strategy resilience against escalating fees, slippages, and risk sizing.
     */
    async runStressTest(strategy, asset = 'BTC/USD', timeframe = '5m', parameters, candleCount = 100, userId = 1) {
        const baseFee = 0.50;
        const baseSlippage = 0.0001;
        // 1. Run Baseline
        const baseline = await backtesting_service_1.backtestingService.runBacktest({
            asset,
            timeframe,
            strategy,
            parameters,
        ...
        for (const mult of costMultipliers) {
            ...
            const isSensitive = mult > 1.0 && (sim.netPnl < 0 || (baselinePnl > 0 && sim.netPnl <= baselinePnl * 0.4));
            if (isSensitive) {
        ...
        try {
            await database_1.pool.query(`INSERT INTO robustness_runs (id, user_id, strategy, run_type, summary_json, details_json, passed)
         VALUES (?, ?, ?, 'STRESS_TEST', ?, ?, ?)`, [
                `rr_${(0, crypto_1.randomUUID)().replace(/-/g, '').substring(0, 16)}`,
                userId,
                strategy,
...
exports.stressTestService = new StressTestService();
