                ...
                this.ws.on('open', async () => {
                    try {
                        ...
                        if (auth.is_virtual !== 1 || !auth.loginid?.startsWith('VRTC')) {
                            this.disconnect();
                            return reject(new Error('[SAFETY VIOLATION] TradePilot connects to Deriv VIRTUAL / DEMO accounts ONLY. Real accounts (CR...) are strictly forbidden.'));
                        }
                        this.accountInfo = {
                            connected: true,
                        ...
                        console.log(`[DerivDemo] Verified Demo Account: ${auth.loginid} (${auth.currency} ${auth.balance}) [SAFETY VERIFIED: DEMO ONLY]`);
