...
exports.demoPromotionService = exports.DemoPromotionService = void 0;
const strategyV2_service_1 = require("./strategyV2.service");
const crypto_1 = __importDefault(require("crypto"));
class DemoPromotionService {
    ...
    config = {
        ...
        description: 'ABC_COMBO active DEMO strategy combining High Volatility 30s duration, Ranging MACD + Bollinger %B confluence, and Low Regime >=80 score threshold.'
    };
    ...
    verifyStartupSafetyGate() {
        ...
        const riskControlsEnabled = this.safetyCircuits.dailyLossLimit.active &&
    ...
    evaluateSignal(candles, indicators, customParams) {
        ...
        const baseResult = strategyV2_service_1.strategyV2Service.evaluateSignal(candles, indicators, customParams);
        ...
        let finalSignal = baseResult.signal;
        // -------------------------------------------------------------
        ...
        if (regime === 'RANGING') {
            ...
            const macdVal = ind.macd.value ?? ind.macd.macd ?? 0;
        ...
        // -------------------------------------------------------------
        const isLowRegime = this.rules.lowRegimes.includes(regime) || regime === 'LOW_VOLATILITY';
        if (isLowRegime) {
            ruleCApplied = true;
    ...
    recordDemoTrade(trade) {
        ...
        if (trade.filterReason) {
            ...
            if (trade.result === 'WIN') {
                ...
                if (this.safetyCircuits.consecutiveLossBreaker.currentConsecutive >
            ...
            const fp = crypto_1.default.createHash('sha256').update(`${trade.asset}-${trade.timestamp}`).digest('hex');
     ...
     */
    rollbackToV2(reason, triggeredBy = 'MANUAL_OPERATOR') {
        this.config.activeStrategy = 'STRATEGY_V2';
        this.config.status = 'ROLLED_BACK';
        const event = {
            id: `RB-${Date.now()}-${crypto_1.default.randomBytes(2).toString('hex')}`,
            timestamp: new Date().toISOString(),
     ...
     */
    restoreAbcCombo(reason, triggeredBy = 'MANUAL_OPERATOR') {
    ...
    calculateWilsonInterval(wins, total) {
        if (total === 0)
...
exports.DemoPromotionService = DemoPromotionService;
exports.demoPromotionService = new DemoPromotionService();
