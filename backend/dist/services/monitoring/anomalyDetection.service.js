...
class AnomalyDetectionService {
     ...
     */
    async checkBalanceAnomaly(asset, balance, tradePnl) {
        if (balance < 0) {
            return this.recordAnomaly({
                type: 'INVALID_VALUES',
                asset,
    ...
    async recordAnomaly(params) {
        const id = `anom_${Date.now()}_${(0, crypto_1.randomUUID)().replace(/-/g, '').substring(0, 6)}`;
        const anomaly = {
            id,
            type: params.type,
        ...
        try {
            await database_1.pool.query(`INSERT INTO anomalies (id, type, asset, severity, observed_value, expected_range, explanation, resolved)
         VALUES (?, ?, ?, ?, ?, ?, ?, FALSE)`, [
                anomaly.id,
                anomaly.type,
                anomaly.asset,
