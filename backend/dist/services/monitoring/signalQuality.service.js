...
exports.signalQualityService = exports.SignalQualityService = void 0;
const crypto_1 = require("crypto");
const database_1 = require("../../config/database");
class SignalQualityService {
    inMemorySignals = new Map();
    ...
    async registerSignal(params) {
        const signalId = `SIG-${Date.now()}-${(0, crypto_1.randomUUID)().replace(/-/g, '').substring(0, 8)}`;
        const signal = {
            signalId,
            asset: params.asset,
        ...
        try {
            await database_1.pool.query(`INSERT INTO signals (id, asset, timeframe, strategy, direction, entry_price, stop_loss, take_profit, risk_amount, confidence, market_regime, indicator_snapshot, status, rejection_reason)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                signal.signalId,
                signal.asset,
                signal.timeframe,
     ...
     */
    async updateSignalStatus(signalId, status, rejectionReason) {
        ...
        sig.status = status;
        if (rejectionReason)
            sig.rejectionReason = rejectionReason;
        this.inMemorySignals.set(signalId, sig);
        try {
            await database_1.pool.query(`UPDATE signals SET status = ?, rejection_reason = ? WHERE id = ?`, [status, sig.rejectionReason || null, signalId]);
        }
        catch {
            // ignore db error
        }
    ...
    async recordSignalOutcome(outcome) {
        ...
        const returnPct = sig && sig.direction === 'SELL'
        ...
        const result = outcome.pnl > 0 ? 'WIN' : 'LOSS';
        const fullOutcome = {
        ...
        try {
            await database_1.pool.query(`INSERT INTO signal_outcomes (signal_id, actual_entry_price, actual_exit_price, return_pct, max_favorable_excursion, max_adverse_excursion, holding_time_seconds, result, pnl)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
     ...
     */
    async getSignalAnalytics(strategy, asset) {
...
exports.SignalQualityService = SignalQualityService;
exports.signalQualityService = new SignalQualityService();
