...
class RiskService {
    static MAX_DAILY_LOSS_PERCENT = 0.05; // 5%
    static MAX_CONCURRENT_TRADES = 1;
    static getRiskPercent(riskLevel) {
        switch (riskLevel) {
            case 'LOW':
                return 0.005; // 0.5%
            case 'MEDIUM':
    ...
    calculateTradeAmount(investmentAmount, riskLevel) {
    ...
    checkRiskRules(startingBalance, currentPnL, activeTradesCount) {
