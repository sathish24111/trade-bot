...
exports.backtestingService = exports.BacktestingService = void 0;
...
const metrics_service_1 = require("./research/metrics.service");
const regime_service_1 = require("./research/regime.service");
const BACKTEST_DISCLAIMER = 'Backtest results are simulated paper trades using historical market data. Past performance is not indicative of future results and does not guarantee profit. Trading involves risk.';
class BacktestingService {
     ...
     */
    async runBacktest(req, persist = true) {
        ...
        if (req.candles && req.candles.length >= 25) {
            ...
            candles = await market_service_1.marketService.getCandles(asset, timeframe, req.candleCount || 120);
        ...
        const spread = req.spread !== undefined ? req.spread : env_1.env.BACKTEST_DEFAULT_SPREAD;
        const slippage = req.slippage !== undefined ? req.slippage : env_1.env.BACKTEST_DEFAULT_SLIPPAGE;
        const fee = req.fee !== undefined ? req.fee : env_1.env.BACKTEST_DEFAULT_FEE;
        ...
        for (let i = 21; i < candles.length; i++) {
            ...
            if (currentPos) {
                ...
                let exitReason = '';
                const priceChangePct = currentPos.direction === 'BUY'
                ...
                if (shouldExit) {
                    ...
                    const finalExitPrice = currentPos.direction === 'BUY'
                        ? rawExitPrice * (1 - spread / 2 - slippage)
                        : rawExitPrice * (1 + spread / 2 + slippage);
                    const returnRate = currentPos.direction === 'BUY'
            ...
            if (!currentPos) {
                const evalResult = strategy_service_1.strategyEngine.evaluateFromCandles(strategyName, historicalSlice, req.parameters);
                if (evalResult.signal === 'BUY' || evalResult.signal === 'SELL') {
                    const entryPrice = evalResult.signal === 'BUY'
            ...
            let unrealizedPnl = 0;
            if (currentPos) {
                const diffPct = currentPos.direction === 'BUY'
                    ? (currentPrice - currentPos.entryPrice) / currentPos.entryPrice
                    : (currentPos.entryPrice - currentPrice) / currentPos.entryPrice;
        ...
        const backtestId = (0, crypto_1.randomUUID)();
        const advancedMetrics = metrics_service_1.metricsService.calculateMetrics(trades, equityCurve, initialBalance);
        // Build regime map for trades
        const regimeMap = new Map();
        for (let i = 21; i < candles.length; i++) {
            const reg = regime_service_1.regimeService.classifyRegime(candles, i);
            regimeMap.set(new Date(candles[i].timestamp).toISOString(), reg.regime);
        }
        const regimeBreakdown = regime_service_1.regimeService.aggregatePerformanceByRegime(trades, regimeMap);
        const result = {
            id: backtestId,
            userId,
            asset,
        ...
        if (persist) {
            ...
            audit_service_1.auditService.log('BACKTEST_COMPLETED', userId, {
                backtestId,
                asset,
                strategy: strategyName,
     ...
     */
    async persistBacktest(result) {
        const connection = await database_1.pool.getConnection();
        try {
            await connection.beginTransaction();
            await connection.query(`INSERT INTO backtest_runs (
          ...
          profit_factor, total_trades
            ...
            for (const t of result.trades) {
                await connection.query(`INSERT INTO backtest_trades (
            ...
            for (let i = 0; i < result.equityCurve.length; i += step) {
                ...
                await connection.query(`INSERT INTO backtest_equity (backtest_id, timestamp, balance, equity)
           VALUES (?, ?, ?, ?)`, [result.id, pt.timestamp, pt.balance, pt.equity]);
            }
     ...
     */
    async compareStrategies(userId, asset, timeframe = '5m', candleCount = 100, initialBalance = 10000) {
     ...
     */
    async getBacktestById(id, userId) {
        const [runs] = await database_1.pool.query('SELECT * FROM backtest_runs WHERE id = ? AND user_id = ?', [id, userId]);
        if (!runs || runs.length === 0)
            return null;
        const run = runs[0];
        const [trades] = await database_1.pool.query('SELECT * FROM backtest_trades WHERE backtest_id = ? ORDER BY timestamp ASC', [id]);
        const [equity] = await database_1.pool.query('SELECT * FROM backtest_equity WHERE backtest_id = ? ORDER BY id ASC', [id]);
        return {
            ...run,
            trades,
            equityCurve: equity,
     ...
     */
    async getUserBacktests(userId, limit = 20) {
        const [runs] = await database_1.pool.query('SELECT * FROM backtest_runs WHERE user_id = ? ORDER BY created_at DESC LIMIT ?', [userId, limit]);
        return runs;
    }
}
exports.BacktestingService = BacktestingService;
exports.backtestingService = new BacktestingService();
