...
export async function initDatabase(): Promise<void> {
  ...
  try {
    ...
    for (const rp of defaultRiskProfiles) {
      await connection.query(
        `INSERT IGNORE INTO risk_profiles (id, name, risk_per_trade, max_daily_loss, max_concurrent_trades, description)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [rp.id, rp.name, rp.risk, rp.maxDaily, rp.maxConcurrent, rp.desc]
      );
    }
    ...
    for (const strat of defaultStrategies) {
      await connection.query(
        `INSERT IGNORE INTO strategies (id, name, description, enabled) VALUES (?, ?, ?, ?)`,
        [strat.id, strat.name, strat.description, true]
      );
    }
    ...
    if (rows.length === 0) {
      ...
      const [insertResult] = await connection.query<mysql.ResultSetHeader>(
        `INSERT INTO users (name, email, mobile, password_hash, demo_balance, account_type)
         VALUES (?, ?, ?, ?, ?, ?)`,
        ['Sathish', 'demo@tradepilot.app', '+91 98765 43210', passwordHash, 10000.00, 'Demo Account']
      );

      const newUserId = insertResult.insertId;

      await connection.query(
        `INSERT IGNORE INTO user_settings (user_id, default_strategy, default_risk, default_duration, notifications_enabled, sound_enabled)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [newUserId, 'EMA_RSI', 'LOW', 15, true, false]
      );

      await connection.query(
        `INSERT IGNORE INTO notification_preferences (user_id, notifications_enabled, critical_risk, strategy_drift, market_data, system_health, paper_trade, experiment, anomaly)
