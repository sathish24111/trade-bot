...
export class AuthorizationService {
   ...
   */
  assertOwnership(requestUserId: number, resourceOwnerId: number, resourceType = 'Resource'): void {
    if (requestUserId !== resourceOwnerId) {
      const err: any = new Error(`${resourceType} not found`);
   ...
   */
  async testCrossUserIsolation(userAId: number, userBId: number): Promise<{ isolated: boolean; testsRan: number }> {
    let testsRan = 0;

    // Test 1: Portfolio isolation
    try {
      const [portfolios] = await pool.query<any[]>(
    ...
    try {
      const [sessions] = await pool.query<any[]>(
        `SELECT id, user_id FROM trading_sessions WHERE user_id = ?`,
        [userBId]
      );
