import { BacktestTrade, BacktestEquityPoint } from '../backtesting.service';
import { AdvancedMetrics, MonthlyPerformance, SampleSizeRating } from '../../models/Research';
...
export class MetricsService {
  ...
  calculateMetrics(
    ...
    let sampleSizeRating: SampleSizeRating = 'ADEQUATE';
    let sampleSizeWarning: string | undefined = undefined;
   ...
   */
  private calculateSharpeAndSortino(trades: BacktestTrade[]): {
    ...
    const tradeReturns: number[] = trades.map(t => (t.amount > 0 ? t.pnl / t.amount : 0));
   ...
   */
  aggregateMonthlyPerformance(trades: BacktestTrade[], initialBalance: number): MonthlyPerformance[] {
    const monthsMap = new Map<string, BacktestTrade[]>();
    ...
    for (const month of sortedMonths) {
      ...
      for (const t of monthTrades) {
        monthBal += t.pnl;
        if (monthBal > peak) peak = monthBal;
   ...
   */
  calculateConfidenceIntervals(trades: BacktestTrade[], confidence = 0.95): {
    winRateInterval: { lower: number; upper: number; isSufficient: boolean };
    expectancyInterval: { lower: number; upper: number; isSufficient: boolean };
  } {
