...
export class OutOfSampleValidationService {
  ...
  private simulateStrategyOnCandles(
    ...
    const trades: Array<{ entryPrice: number; exitPrice: number; pnl: number; result: 'WIN' | 'LOSS'; signalScore: number }> = [];
    ...
    for (let i = 25; i < candles.length - 1; i++) {
      ...
      const evalResult = strategyV2Service.evaluateSignal(historicalSlice, undefined, params);
  ...
  public runValidation(
    ...
    for (const score of testScores) {
      for (const cd of testCooldowns) {
        const testParam: StrategyV2Parameters = { ...params, minSignalScore: score, cooldownSeconds: cd };
        ...
        let cliffSeverity: 'MODERATE' | 'SEVERE' | undefined = undefined;
    ...
    // 4. Overall Verdict
    let verdict: 'PASS' | 'MARGINAL' | 'FAIL' = 'PASS';
    ...
    if (totalCliffsDetected >= 3) {
      if (verdict === 'PASS') verdict = 'MARGINAL';
  ...
  private generateSyntheticCandles(count: number): Candle[] {
...
export const outOfSampleValidationService = new OutOfSampleValidationService();
