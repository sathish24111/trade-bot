...
import { PaperTradeJournalEntry, MarketRegimeV2 } from '../../models/StrategyV2';
...
class PaperJournalService {
  ...
  public async ensureTable(): Promise<void> {
    ...
    try {
      ...
      for (const colDef of cols) {
        ...
        try {
          await pool.query(`ALTER TABLE paper_trade_journal ADD COLUMN ${colDef}`);
        } catch {}
      }
  ...
  public async recordEntry(entry: PaperTradeJournalEntry): Promise<void> {
    const enrichedEntry: PaperTradeJournalEntry = {
      ...entry,
      emaScore: entry.emaScore ?? (entry.scoreBreakdown as any)?.emaScore ?? 0,
      rsiScore: entry.rsiScore ?? (entry.scoreBreakdown as any)?.rsiScore ?? 0,
      macdScore: entry.macdScore ?? (entry.scoreBreakdown as any)?.macdScore ?? 0,
      bollingerScore: entry.bollingerScore ?? (entry.scoreBreakdown as any)?.bollingerScore ?? 0,
      momentumScore: entry.momentumScore ?? (entry.scoreBreakdown as any)?.momentumScore ?? 0,
      volatilityScore: entry.volatilityScore ?? (entry.scoreBreakdown as any)?.volatilityScore ?? 0,
   ...
   */
  public async seedValidationDataset(count = 150): Promise<PaperTradeJournalEntry[]> {
    const assets = ['R_100', 'R_50', 'EUR/USD', 'GBP/USD'];
    const regimes: MarketRegimeV2[] = ['TRENDING_UP', 'TRENDING_DOWN', 'RANGING', 'HIGH_VOLATILITY', 'LOW_VOLATILITY'];
    const durations = [5, 15, 30, 60];
    const createdEntries: PaperTradeJournalEntry[] = [];
    const now = Date.now();
    ...
    for (let i = 0; i < count; i++) {
      ...
      const direction: 'BUY' | 'SELL' = regime === 'TRENDING_UP'
        ? 'BUY'
      ...
      const entry: PaperTradeJournalEntry = {
        id: `DEMO-${Date.now()}-${i}-${crypto.randomBytes(2).toString('hex')}`,
