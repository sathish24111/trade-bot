...
import { MarketRegimeType, RegimeClassification, RegimePerformance } from '../../models/Research';
...
export class RegimeService {
   ...
   */
  classifyRegime(candles: Candle[], currentIndex: number): RegimeClassification {
    ...
    const indicators = indicatorService.calculateAllIndicators(historicalSlice);
    ...
    for (let j = 0; j < lookback; j++) {
      ...
      if (idx >= 15) {
        ...
        const subIndicators = indicatorService.calculateAllIndicators(subSlice);
   ...
   */
  aggregatePerformanceByRegime(
    trades: BacktestTrade[],
    regimeMap: Map<string, MarketRegimeType>
  ): RegimePerformance[] {
    const regimes: MarketRegimeType[] = ['TRENDING', 'RANGING', 'HIGH_VOLATILITY', 'LOW_VOLATILITY'];
    const buckets = new Map<MarketRegimeType, BacktestTrade[]>();
