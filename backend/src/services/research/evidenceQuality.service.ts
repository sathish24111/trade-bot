...
import { EvidenceQualityLevel, StrategyEvidenceItem } from '../../models/Phase9';
...
export class EvidenceQualityService {
   ...
   */
  async evaluateStrategyEvidence(strategyId: string): Promise<StrategyEvidenceItem> {
    ...
    let score = 0;
    if (item.datasetQuality === 'VALID') score += 15;
    if (item.sampleSizeRating === 'LARGER_SAMPLE') score += 25;
    else if (item.sampleSizeRating === 'MODERATE') score += 15;
    else score += 5;

    if (item.oosTested) score += 15;
    if (item.walkForwardTested) score += 15;
    if (item.stressTested) score += 10;
    if (item.monteCarloSimulated) score += 10;
    if (item.parameterStabilityTested) score += 10;

    let overallLevel: EvidenceQualityLevel = 'LIMITED';
    if (score >= 80) overallLevel = 'SUBSTANTIAL';
    else if (score >= 50) overallLevel = 'MODERATE';
    else if (score >= 25) overallLevel = 'LIMITED';
    else overallLevel = 'VERY_LIMITED';

    item.overallEvidenceLevel = overallLevel;

    try {
      await pool.query(
        `INSERT INTO research_evidence_metrics
  ...
  async getEvidenceMatrix(): Promise<StrategyEvidenceItem[]> {
    const strategies = ['EMA_RSI', 'MACD', 'BOLLINGER_BANDS', 'MULTI_INDICATOR'];
    const matrix: StrategyEvidenceItem[] = [];
