...
export class DatasetService {
  private inMemoryDatasets: Map<string, ResearchDataset> = new Map();

  /**
   * Helper to parse timeframe string into expected interval in seconds.
   */
  getTimeframeSeconds(timeframe: string): number {
   ...
   */
  validateCandles(
    rawCandles: HistoricalCandle[],
    timeframe: string
  ): { cleanedCandles: HistoricalCandle[]; report: DatasetValidationReport } {
    ...
    const validBars: HistoricalCandle[] = [];
  ...
  partitionDataset(
    ...
    valRatio = 0.15,
    testRatio = 0.15
  ): { train: HistoricalCandle[]; val: HistoricalCandle[]; test: HistoricalCandle[] } {
  ...
  async registerDataset(
    ...
    asset: string,
    timeframe: string,
    candles: HistoricalCandle[] | Candle[],
    source = 'simulated'
  ): Promise<{ metadata: DatasetMetadata; validationReport: DatasetValidationReport; dataset: ResearchDataset }> {
    const normCandles: HistoricalCandle[] = candles.map((c) => ({
      ...
      timeframe,
      source: (c as any).source || source,
      isComplete: true
    }));

    const { cleanedCandles, report } = this.validateCandles(normCandles, timeframe);

    const id = `ds_${randomUUID().replace(/-/g, '').substring(0, 16)}`;
    ...
    try {
      await pool.query(
        `INSERT INTO historical_datasets (id, name, asset, timeframe, candle_count, start_time, end_time, sha256_checksum, is_validated)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id,
          name,
      ...
      await pool.query(
        `INSERT INTO dataset_validation_results (id, dataset_id, passed, total_candles, missing_candles, duplicate_timestamps, out_of_order_candles, gap_count, max_gap_duration_sec, report_json)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          `val_${randomUUID().replace(/-/g, '').substring(0, 16)}`,
          id,
          report.passed,
          report.totalCandles,
   ...
   */
  async getDataset(id: string): Promise<ResearchDataset | null> {
    ...
    try {
      const [rows] = await pool.query<any[]>(
        `SELECT * FROM historical_datasets WHERE id = ?`,
        [id]
      );
   ...
   */
  async listDatasets(asset?: string, timeframe?: string): Promise<DatasetMetadata[]> {
    const list: DatasetMetadata[] = [];

    // From memory
    for (const ds of this.inMemoryDatasets.values()) {
      if (asset && ds.metadata.asset !== asset) continue;
      if (timeframe && ds.metadata.timeframe !== timeframe) continue;
