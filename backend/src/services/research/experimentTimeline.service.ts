...
import { ExperimentTimelineEvent } from '../../models/Phase7';
import { broadcastEvent } from '../../websocket/websocket.server';

export class ExperimentTimelineService {
  private sequenceCounter: Map<string, number> = new Map();
  private inMemoryTimeline: Map<string, ExperimentTimelineEvent[]> = new Map();
  ...
  async recordEvent(params: {
  ...
  }): Promise<ExperimentTimelineEvent> {
    ...
    const event: ExperimentTimelineEvent = {
    ...
    try {
      const [res] = await pool.query<any>(
        `INSERT INTO experiment_timeline_events (experiment_id, sequence, event_type, title, details, timestamp)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [event.experimentId, event.sequence, event.eventType, event.title, JSON.stringify(event.details), timestamp]
      );
   ...
   */
  async getTimeline(experimentId: string): Promise<ExperimentTimelineEvent[]> {
    ...
    try {
      const [rows] = await pool.query<any[]>(
        `SELECT id, experiment_id, sequence, event_type, title, details, timestamp
    ...
    try {
      const [trades] = await pool.query<any[]>(
        `SELECT id, asset, direction, amount, entry_price, exit_price, pnl, result, strategy, created_at
...
export const experimentTimelineService = new ExperimentTimelineService();
