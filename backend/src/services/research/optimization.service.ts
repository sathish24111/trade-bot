...
const OPTIMIZATION_DISCLAIMER =
  'Optimization results are simulated paper trades using historical market data. Optimization tests multiple historical parameter combinations; it does NOT predict future market behavior or guarantee profit.';

export class OptimizationService {
  /**
   * Generates Cartesian product combinations from parameter ranges dictionary.
   */
  generateCombinations(ranges: Record<string, number[]>): StrategyParameters[] {
    const keys = Object.keys(ranges);
    if (keys.length === 0) return [{}];

    const results: StrategyParameters[] = [];

    const helper = (depth: number, current: StrategyParameters) => {
   ...
   */
  async runOptimization(config: OptimizationRunConfig): Promise<OptimizationRunResult> {
    const {
      userId,
      asset,
      timeframe = '5m',
    ...
    if (candles.length < 75) {
      throw new Error(
        `Insufficient candle data for 3-way split optimization. Found ${candles.length} candles, minimum 75 required.`
      );
    }
    ...
    const combinationResults: OptimizationCombinationResult[] = [];

    for (let i = 0; i < combinations.length; i++) {
      const params = combinations[i];
      const combId = `comb-${i + 1}-${randomUUID().substring(0, 8)}`;

      // A. In-Sample Training Backtest
      const trainRes = await backtestingService.runBacktest({
        userId,
        asset,
        timeframe,
        strategy,
      ...
      const trainMetrics = metricsService.calculateMetrics(trainRes.trades, trainRes.equityCurve, initialBalance);

      // B. Validation Backtest
      const valRes = await backtestingService.runBacktest({
        userId,
        asset,
        timeframe,
        strategy,
      ...
      const valMetrics = metricsService.calculateMetrics(valRes.trades, valRes.equityCurve, initialBalance);

      // C. Out-Of-Sample Test Backtest
      const testRes = await backtestingService.runBacktest({
        userId,
        asset,
        timeframe,
        strategy,
      ...
      const testMetrics = metricsService.calculateMetrics(testRes.trades, testRes.equityCurve, initialBalance);
    ...
    let overfittingWarning: string | undefined = undefined;
   ...
   */
  private async persistOptimizationRun(run: OptimizationRunResult): Promise<void> {
  ...
  async generateSensitivityHeatmap(config: {
    ...
    const matrix: HeatmapPoint[] = [];
    const candles = await marketService.getCandles(asset, timeframe, candleCount);
    const cleaned = backtestingService.validateAndCleanCandles(candles);
    ...
    for (const p1 of param1Range) {
      for (const p2 of param2Range) {
        ...
        const metricVal =
          sim.advancedMetrics && sim.advancedMetrics.sharpeRatio !== null
