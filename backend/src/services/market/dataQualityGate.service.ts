...
import { NormalizedMarketTick, DataQualityValidation } from '../../models/Phase7';

export class DataQualityGateService {
  private lastTicksByAsset: Map<string, NormalizedMarketTick> = new Map();
  private lastCandlesByAsset: Map<string, Candle> = new Map();

  /**
   * Validates a single incoming market tick
   */
  validateTick(tick: NormalizedMarketTick): DataQualityValidation {
    const now = Date.now();
    const result: DataQualityValidation = {
      isValid: true,
      timestampOk: true,
      priceOk: true,
      ohlcOk: true,
   ...
   */
  validateCandle(asset: string, candle: Candle): DataQualityValidation {
    const now = Date.now();
    const result: DataQualityValidation = {
      isValid: true,
      timestampOk: true,
...
export const dataQualityGate = new DataQualityGateService();
