...
export class RequestMetricsService {
  private routeMap: Map<string, RouteMetric> = new Map();
  private allLatencies: number[] = [];
  private totalRequests = 0;
  private totalErrors = 0;
  private readonly MAX_LATENCY_SAMPLES = 5000;

  recordRequest(method: string, endpoint: string, statusCode: number, durationMs: number): void {
    this.totalRequests++;
    if (statusCode >= 400) {
      this.totalErrors++;
    }
