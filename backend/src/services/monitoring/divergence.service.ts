...
export class DivergenceService {
   ...
   */
  async analyzeDivergence(experimentId: string): Promise<DivergenceReport> {
    const exp = await experimentService.getExperiment(experimentId);
    if (!exp) {
    ...
    let classification: DivergenceClassification = 'NO_SIGNIFICANT_DIVERGENCE';
