...
export const DEFAULT_STRATEGY_V2_PARAMS: StrategyV2Parameters = {
  emaFast: 9,
  emaSlow: 21,
  emaTrend: 50,
  rsiPeriod: 14,
...
const STRATEGY_V2_DISCLAIMER =
  'Strategy V2 confidence and normalized quality scores (0-100) are algorithmically computed for research in DEMO/PAPER mode. Score does not represent probability of profit. No signal guarantees winning trades.';
...
export class StrategyV2Service {
   ...
   */
  public classifyRegime(candles: Candle[], currentIndex: number = candles.length - 1): MarketRegimeV2 {
    ...
    const indicators = indicatorService.calculateAllIndicators(slice);
    ...
    for (let j = 0; j < lookback; j++) {
      ...
      if (idx >= 15) {
        ...
        const subInd = indicatorService.calculateAllIndicators(subSlice);
    ...
    if (slice.length >= 26) {
      ...
      const prevEmaArr = indicatorService.calculateEMA(prevSliceCloses, 21);
   ...
   */
  public computeSignalScore(
    direction: 'BUY' | 'SELL',
    price: number,
    candles: Candle[],
    indicators: TechnicalIndicators,
    regime: MarketRegimeV2,
    params: StrategyV2Parameters = DEFAULT_STRATEGY_V2_PARAMS
  ): SignalScoreBreakdown {
    ...
    const contributingIndicators: string[] = [];
    ...
    if (emaScore >= 15) confirmationsCount++;
    ...
    if (rsiScore >= 12) confirmationsCount++;
    ...
    if (macdScore >= 10) confirmationsCount++;
    ...
    if (bollingerScore >= 10) confirmationsCount++;
    ...
    if (momentumScore >= 6) confirmationsCount++;
  ...
  public evaluateSignal(
    candles: Candle[],
    indicators?: TechnicalIndicators,
    customParams?: Partial<StrategyV2Parameters>
  ): StrategyV2SignalResult {
    const params: StrategyV2Parameters = { ...DEFAULT_STRATEGY_V2_PARAMS, ...customParams };

    if (!candles || candles.length === 0) {
      const defaultInd = indicatorService.calculateAllIndicators([]);
      const emptyScore: SignalScoreBreakdown = {
        emaScore: 0,
        rsiScore: 0,
        macdScore: 0,
        bollingerScore: 0,
    ...
    let chosenDirection: 'BUY' | 'SELL' | null = null;
    let chosenBreakdown: SignalScoreBreakdown = buyScore;
   ...
   */
  public isSignalInvalidated(
    originalSignal: StrategyV2SignalResult,
    currentPrice: number,
    currentIndicators: TechnicalIndicators,
    currentRegime: MarketRegimeV2
...
export const strategyV2Service = new StrategyV2Service();
